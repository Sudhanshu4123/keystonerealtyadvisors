...
const PROJECT_TYPES = ['Residential', 'Commercial', 'Mixed-Use', 'Industrial', 'Land & Plots'];
...
export default function AdminProjectsPage() {
  ...
  const fetchAdminProjects = async () => {
    setLoading(true);
    try {
      const res = await adminFetch('/projects?publishedOnly=false');
  ...
  const openEditModal = (proj: any) => {
    ...
    setForm({
      ...
      images: proj.images ? proj.images.map((i: any) => ({ imageUrl: i.imageUrl, isPrimary: i.isPrimary })) : [],
      amenities: proj.amenities ? proj.amenities.map((a: any) => (typeof a === 'string' ? a : a.name)) : [],
      floorPlans: proj.floorPlans ? proj.floorPlans.map((fp: any) => ({ title: fp.title, description: fp.description || '', fileUrl: fp.fileUrl })) : [],
      brochures: proj.brochures ? proj.brochures.map((b: any) => ({ fileName: b.fileName, fileUrl: b.fileUrl })) : [],
    });
    setErrorMsg('');
    setIsModalOpen(true);
  };
  ...
  const executeDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await adminFetch(`/projects/${deleteTargetId}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleTogglePublish = async (proj: any) => {
    try {
      await adminFetch(`/projects/${proj.id}`, {
        method: 'PUT',
            ...
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {filteredProjects.map((proj) => {
                  const primaryImg =
                    proj.images?.find((i: any) => i.isPrimary)?.imageUrl ||

...
import { Settings as SettingsIcon, Save, CheckCircle2, Loader2, BarChart3, MapPin } from 'lucide-react';
...
export default function AdminSettingsPage() {
    ...
    try {
      ...
      if (res.ok && settings) {
        setForm({
          ...
          projectsDelivered: settings.projectsDelivered || '48+',
          yearsExperience: settings.yearsExperience || '15+',
          happyCustomers: settings.happyCustomers || '12,500+',
          ongoingProjects: settings.ongoingProjects || '12',
        });
      }
    } catch (err) {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Website settings, Google Maps link, and company metrics saved successfully!');
    ...
    <div className="space-y-6 max-w-4xl bg-slate-50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">
            Configure company phone numbers, email, address, Google Maps Business link, statistics counters, and SEO metadata.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSaveSettings} className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm text-xs font-medium text-slate-900">
        ...
        <div className="space-y-4">
          ...
          {/* Google Maps / Business Profile Link Field */}
          <div className="space-y-1 pt-2">
            <label className="font-bold text-slate-700 flex items-center gap-1.5">
              <MapPin className="w-4 h-4 text-amber-600" />
              <span>Google Business Profile / Google Maps Embed URL</span>
            </label>
            <input
              type="text"
              placeholder="Paste your Google Business Profile Share URL or Embed iframe src link..."
              value={form.googleMapsUrl}
              onChange={(e) => setForm({ ...form, googleMapsUrl: e.target.value })}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-blue-900 font-mono text-[11px]"
            />
            <p className="text-[10px] text-slate-500">
              Tip: Paste your exact Google Business Profile embed URL or share link here to render your business map on the website contact page.
            </p>
          </div>
        </div>

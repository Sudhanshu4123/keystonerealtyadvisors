...
export default function GoogleMapPlaceholder({ location, address, city, googleMapsUrl }: GoogleMapProps) {
  const query = encodeURIComponent(`${address || ''} ${location} ${city || ''}`);
  
  // If custom embed URL provided in settings/project, use it; otherwise compute dynamic query embed URL
  const mapsEmbedUrl = googleMapsUrl && googleMapsUrl.includes('http')
    ? googleMapsUrl
    : `https://maps.google.com/maps?q=${query}&t=&z=14&ie=UTF8&iwloc=&output=embed`;
    
  const externalMapsUrl = googleMapsUrl && !googleMapsUrl.includes('embed')
    ? googleMapsUrl
    ...
    <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm space-y-3">
      <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-bold text-slate-900">Google Business Location Map</h4>
        </div>
        <a
      ...
      <div className="relative w-full h-80 bg-slate-100">
        <iframe
          title="Google Business Location Map"
          src={mapsEmbedUrl}
